...
library(tidyverse)
library(psych)
load("C:/Users/76519/Documents/tech_data.Rdata")
```
...
which(colnames(tech_food)=="DAYNUM")
...
nutr_with_age_BOTHDAYS = tech_nutr |>#这一列里基本都是
  group_by(ABSPID) |>
...
```{r}
library(stringr)
which(colnames(tech_nutr)=="Category.code")
fooditems_dict = read.csv("C:/Users/76519/Documents/tech_nutr.csv") |>
...
print(fa_grouped_result$Vaccounted)
...
Now, Lea will follow your instructions to finish the third item of "Next steps":
First of all, I find the data of total energy consumed and weight from both "tech_nutr" and "tech_biom" dataset, then combine them with the dataset that you have claned. Of course, it is necessary to remove the rows with outliers,such as "NA" in column BMI and "0" in column Both "ENERGYT1" and "ENERGYT2."
```{r}
anyNA(tech_biom$ENERGYT1)
anyNA(tech_biom$ENERGYT2)
groupd_items_with_BMI <- wide_adults_food_servings_foods_grouped |>
  left_join(y = tech_biom |> select(ABSPID, PHDKGWBC), by = "ABSPID") |>
  filter(!is.na(PHDKGWBC)) |>
  left_join(y = tech_nutr|> select(ABSPID, ENERGYT1,ENERGYT2), by = "ABSPID")
any(groupd_items_with_BMI$PHDKGWBC == 0)
which(groupd_items_with_BMI$ENERGYT1 == 0)
which(groupd_items_with_BMI$ENERGYT2 == 0)
groupd_items_with_BMI_filtered <- groupd_items_with_BMI |>
  filter(!row_number() %in% c(580, 1606, 2145))
groupd_items_BMITotalEnergy <- groupd_items_with_BMI_filtered |>
  mutate(TotalENERGY = ENERGYT1 + ENERGYT2) |>
  select(-ENERGYT1,-ENERGYT2)
```
Because the dataset is changing, I will re-factor the data.
```{r}
groupd_items_BMITotalEnergy_1 <- groupd_items_BMITotalEnergy |>
  select(-26,-29,-16,-31,-39,-9) 
kmo_result <- KMO(groupd_items_BMITotalEnergy |> select(-ABSPID,-PHDKGWBC,-TotalENERGY))
kmo_result$MSA
fa_grouped_result_2 = fa(groupd_items_BMITotalEnergy_1 |> select(-ABSPID,-PHDKGWBC,-TotalENERGY) |> scale(), fm = "pa", nfactors = 2, rotate = "varimax")
#find the loading matrix
print(fa_grouped_result_2$loadings)
```
By observing the load, it can be concluded that factor 1 represents high-calorie and meat foods, and factor 2 represents low-calorie foods such as fruits and vegetables.Next, we extract the factor score and predict the BMI by these two factors and the total energy expenditure.
```{r}
head(fa_grouped_result_2$scores)
factor_scores <-fa_grouped_result_2$scores
Linear_regression <- cbind(factor_scores, TotalEnergy = groupd_items_BMITotalEnergy$TotalENERGY, WG = groupd_items_BMITotalEnergy$PHDKGWBC)
  
mydata_scaled <- as.data.frame(scale(Linear_regression))
lm_result <- lm(WG~ PA1 + PA2 + TotalEnergy, data = mydata_scaled)
print(lm_result)
summary(lm_result)
